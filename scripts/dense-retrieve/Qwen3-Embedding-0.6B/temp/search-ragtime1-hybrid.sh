...
# NOTE: mirrors search-neuclir1-hybrid.sh's shard-group construction (one group
# per shard FILE, streamed one at a time so peak memory stays bounded -- not one
# group per language: a language's shards can total 100-200G, which blew past
# even 512G) and scripts/retrieve/neuclir1-hybrid-claim-doc.sh's alpha sweep
# (0.0 = pure doc-level, 0.5 = evenly split).
