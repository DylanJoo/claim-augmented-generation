#!/bin/sh
#SBATCH --job-name=search-neuclir1-cc-kmeans-core-sanity
#SBATCH --output=logs/search-neuclir1-cc-kmeans-core-sanity.out
#SBATCH --error=logs/search-neuclir1-cc-kmeans-core-sanity.err
...
for ALPHA in 0.6 0.7 0.8 0.9; do
    python pipeline/run_cc_kmeans.py \
        ...
        --output runs/sanity-relevant-only/reranked/run.neuclir1.documents.Qwen3-Embedding-0.6B.relevant-only.cckmeans-core.top${TOP_M}-k${N_CLUSTERS}-${LABEL_MODE}.alpha-${ALPHA}.txt \
        --k 20 \
        --alpha ${ALPHA} \
        --kmeans-top-m ${TOP_M} \
