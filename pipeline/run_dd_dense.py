...
pre-computed doc-level TREC run file, followed by a greedy Maximal Marginal
Relevance re-ranking of the evidence pool, exactly as in run_dd.py. The
difference: sim(d, d') is a dot product of pre-computed tevatron doc-level
embeddings instead of a local BM25 self-similarity score. See
src/retrieval/dd_dense.py.
...
Usage:
    python pipeline/run_dd_dense.py \
        ...
        --output <results.txt> \
        [--k 1000] [--lambda-mult 0.9] [--tag dd-dense]
"""
