...
Usage:
    python pipeline/run_cc_kmeans_weighted.py \
        --topics <topics.jsonl> \
...
from retrieval import cc_kmeans_weighted
...
def main():
    ...
    parser.add_argument("--kmeans-rel-tau", type=float, default=None,
                        help="Relevance-weight the k-means fit: each claim gets its parent doc's weight "
                             "softmax(base score / tau), in run-file score units (cosine-scale scores: "
                             "~0.005-0.1; smaller = more skewed towards top docs). Unset (default): "
                             "unweighted k-means")
    parser.add_argument("--kmeans-stages", default=None,
                        help="Staged re-clustering: comma-separated top_m:n_clusters[:rel_tau], e.g. "
                             "'20:20,40:30'. Fit stage 0 up front; once --kmeans-stage-trigger of the "
                             "clusters are touched by selected docs, re-cluster from scratch on the "
                             "next (larger) core. Overrides --kmeans-top-m / --kmeans-n-clusters "
                             "(default: single stage)")
    parser.add_argument("--kmeans-stage-trigger", type=float, default=1.0,
                        help="Fraction of clusters that must be touched by selected docs before the "
                             "next stage re-clusters (default: 1.0 = every cluster touched)")
    ...
    parser.add_argument("--tag", default="cc-kmeans-weighted",
                        help="Run tag written in the TREC output (default: cc-kmeans-weighted)")
    ...
    stages = cc_kmeans_weighted.parse_stages(args.kmeans_stages) if args.kmeans_stages else None
    if stages:
        logger.info("staged re-clustering %s, trigger=%.2f (overrides --kmeans-top-m/--kmeans-n-clusters)",
                    stages, args.kmeans_stage_trigger)
    results = cc_kmeans_weighted.run(
        inputs,
        ...
        rel_tau=args.kmeans_rel_tau,
        stages=stages,
        stage_trigger=args.kmeans_stage_trigger,
    )
