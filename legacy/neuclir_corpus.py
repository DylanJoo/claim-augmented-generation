...
from decontext.prompts import system_prompt, user_prompt_en_no_title
    ...
    for lang in corpus:
        ...
        with open(output_file, 'a') as f:
            ...
            for batch_docids in tqdm(batch_iterator(docids, size=96), desc=f"Processing {lang} corpus"):
                ...
                batch_titles = [corpus[lang][docid]['title'] for docid in batch_docids]
                batch_texts = [corpus[lang][docid]['text'] for docid in batch_docids]

                # prepare prompts
                if args.use_translation:
                    user_prompts = [user_prompt_en.format(lang=lang, title=title, text=text) \
                            for title, text in zip(batch_titles, batch_texts)]
                else:
                    lang_ = {'zho': 'Chinese', 'rus': 'Russian', 'fas': 'Persian'}[lang]
                    user_prompts = [user_prompt_lang.format(lang=lang_, title=title, text=text) \
                            for title, text in zip(batch_titles, batch_texts)]
