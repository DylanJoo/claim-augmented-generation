...
Identify which item in the Candidate Set is both novel to the Collected Set and is insightful with respect to the Report Request. ONLY select the most crucial information from Candidate Set. Write only the identifier of the information in this format: [number], write None if no items meet both criteria, do not write any other text."""
...
def run(inputs, limit=5, max_char_limit=10000, use_litellm=False):
    ...
    for i, input in tqdm(enumerate(inputs), desc='Compile cluster', total=len(inputs)):
        ...
        while char_count <= max_char_limit and available > 0:
            available = sum([len(cluster) for cluster in input.clusters])
            print(f"Available items:", [len(c) for c in input.clusters], f"char count: {char_count}")

            ### go through top-1 in each cluster 
            for j, cluster in enumerate(input.clusters):
                ...
                if 'none' in llm_output.lower() or len(cluster) == 0: # non relevant or empty cluster
                    input.clusters[j] = {k: v for i, (k, v) in enumerate(cluster.items(), start=1) if i > limit}
                else:
